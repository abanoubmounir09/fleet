...
class Asset(AccountsController):
    ...
    def validate_asset_and_reference(self):
        if self.purchase_invoice or self.purchase_receipt:
            ...
            if reference_doc.get('company') != self.company:
                frappe.throw(_("Company of asset {0} and purchase document {1} doesn't matches.").format(self.name,
                                                                                                         reference_doc.get(
                                                                                                             'name')))
    ...
    def make_asset_movement(self):
        ...
        if reference_docname:
            posting_date, posting_time = frappe.db.get_value(reference_doctype, reference_docname,
                    ...
                    if self.allow_monthly_depreciation:
                        ...
                        for r in range(month_range):
                            if (has_pro_rata and n == 0):
                                ...
                                if r == 0:
                                    days_until_first_depr = date_diff(monthly_schedule_date,
                                                                      self.available_for_use_date)
        ...
        for i, d in enumerate(self.get("schedules")):
            ...
            if straight_line_idx and i == max(straight_line_idx) - 1:
                ...
                depreciation_amount += flt(value_after_depreciation -
                                           flt(book.expected_value_after_useful_life),
    ...
    def validate_expected_value_after_useful_life(self):
        for row in self.get('finance_books'):
            accumulated_depreciation_after_full_schedule = [d.accumulated_depreciation_amount
                                                            for d in self.get("schedules") if
                                                            cint(d.finance_book_id) == row.idx]
            ...
            if accumulated_depreciation_after_full_schedule:
                ...
                if (row.expected_value_after_useful_life and
                    ...
                    frappe.throw(
                        _("Depreciation Row {0}: Expected value after useful life must be greater than or equal to {1}")
    ...
    def get_purchase_document(self):
        asset_bought_with_invoice = self.purchase_invoice and frappe.db.get_value('Purchase Invoice',
                                                                                  self.purchase_invoice, 'update_stock')
    ...
    def get_depreciation_rate(self, args, on_validate=False):
        ...
        if args.get("depreciation_method") == "Written Down Value":
            ...
            no_of_years = flt(
                args.get("total_number_of_depreciations") * flt(args.get("frequency_of_depreciation"))) / 12
...
def transfer_asset(args):
    ...
    frappe.msgprint(_("Asset Movement record {0} created").format(
        "<a href='#Form/Asset Movement/{0}'>{0}</a>".format(movement_entry.name)))
...
def make_asset_movement(assets, purpose=None):
    ...
    for asset in assets:
        ...
        asset_movement.append("assets", {
            ...
            'from_is_driver':asset.get('is_driver'),
            'from__driver':asset.get('driver')
        })
